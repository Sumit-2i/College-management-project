...
def doLogin(request):
    ...
    password=id=request.GET.get('password')
    print(email_id)
    print(password)
    print(request.user)
...
def doRegistration(request):
    ...
    if password != confirm_password:
        ...
        return render(request, 'Both password should match!!')
    is_user_exists=CustomUser.objects.filter(email=email_id).exists()
    if is_user_exists:
        messages.error(request, 'User with this email id already exists. Pleaase proceed to login!!')
        return render(request, 'registration.html')
